...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.339-alpha'));
    ...
    expect(a.behindBy).toBe(59);  // 339-280=59 (tracks app version)
