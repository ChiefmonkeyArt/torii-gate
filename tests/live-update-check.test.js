...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.328-alpha'));
    ...
    expect(a.behindBy).toBe(48);  // 328-280=48 (tracks app version)
