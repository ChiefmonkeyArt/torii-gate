...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.316-alpha'));
    ...
    expect(a.behindBy).toBe(36);  // 316-280=36 (tracks app version)
