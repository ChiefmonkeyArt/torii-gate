...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.330-alpha'));
    ...
    expect(a.behindBy).toBe(50);  // 330-280=50 (tracks app version)
