...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.296-alpha'));
    ...
    expect(a.behindBy).toBe(16);  // 296-280=16 (tracks app version)
