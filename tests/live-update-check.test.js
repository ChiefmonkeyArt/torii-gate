...
describe('checkForUpdateLive', () => {
  it('latest > installed → behind, fetched once, then cached (no 2nd fetch)', async () => {
    const s = memStorage();
    const fetcher = vi.fn(async () => release('v0.2.298-alpha'));
    ...
    expect(a.behindBy).toBe(18);  // 298-280=18 (tracks app version)
