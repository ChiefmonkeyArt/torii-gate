...
- **Debug API cleanup**: ✅ DONE — `window.ToriiDebug` namespace (`engine/debug/toriiDebug.js`); `_mirrorMesh` is the last legacy functional global, preserved and mirrored under the namespace. `_onBotHit` (v0.2.117) and `_grassMat`/`_flowerMat` (v0.2.118) are now DEPRECATED debug aliases only — internal code routes through the event bus / foliage registry respectively, surfaced under ToriiDebug via injected accessors.
