...
- **Debug API cleanup**: ✅ DONE — `window.ToriiDebug` namespace (`engine/debug/toriiDebug.js`); legacy functional globals (`_grassMat`, `_flowerMat`, `_mirrorMesh`) preserved and mirrored under the namespace. `_onBotHit` is now DEPRECATED (v0.2.117) — kept only as a debug-tap alias forwarding onto `EV.BOT_HIT_BY_PLAYER`; internal code routes through the bus.
...
11. 🚧 Implement event bus — seam formalised in v0.2.116; bot-hit bridge migrated in v0.2.117 (`EV.BOT_HIT_BY_PLAYER` from weapons→main, `window._onBotHit` deprecated to a forwarding alias, regression check 9); next, migrate the shader-mat refs (`window._grassMat`/`_flowerMat`) onto the bus, add real `PHASE_CHANGE` subscribers, and emit `WS_*` when netcode lands.
