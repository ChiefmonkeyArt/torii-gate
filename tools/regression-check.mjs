...
const EXPECTED_VERSION = 'v0.2.292-alpha';
...
console.log('[16] CSP via HTTP header + vendored Draco (S3+S4)');
{
  ...
  if (!existsSync(distHtmlP)) {
    ...
    if (!existsSync(headersP)) fail('dist/_headers missing (vite CSP plugin did not run)');
      ...
      // v0.2.292: _headers now carries the sha recomputed from the emitted inline script
    ...
    // carries THAT sha (self-consistency). v0.2.292: the inline import now carries a
