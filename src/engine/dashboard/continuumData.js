...
export const CONTINUUM_VERSION = 'v0.2.203-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.203-alpha (build truth; live trails — manual deploy)' },
    ...
    { label: 'Active slice', value: 'v0.2.203 MVP MANUAL PLAYTEST CHECKLIST (docs/tooling only, no runtime change) — a pure/local manual QA acceptance checklist (tools/playtestChecklist.mjs + CLI tools/playtest-checklist.mjs, npm run playtest:checklist) a human (or future AI handoff) runs by hand against the live build. 17 curated items across 13 areas (launch/title, shooter loop, movement/footsteps, aim/hit-feedback/headshots/body-shots, reload feel, gun/reflection, mirror, crates/physics-nudge, NAP monkey, Continuum dashboard, release-metadata/update-prompt, Nostr read surfaces, gateway portal/travel-confirm shell) — each with reproduction steps, an expected result, a severity (blocker/major/minor), an "if it fails" action, and Result/Notes fields. Emits text/--json/--markdown plus an opt-in bounded in-repo --write → MVP_PLAYTEST_CHECKLIST.md. MANUAL CHECKLIST ONLY: no browser automation, no network, no deploy/publish. +15 unit tests. NON-GOALS held: read-only; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
