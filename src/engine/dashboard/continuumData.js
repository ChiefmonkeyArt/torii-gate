...
export const CONTINUUM_VERSION = 'v0.2.202-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.202-alpha (build truth; live trails — manual deploy)' },
    ...
    { label: 'Active slice', value: 'v0.2.202 MVP PROOF RELEASE NOTES DRAFT (docs/tooling only, no runtime change) — a pure/local release-notes draft (tools/releaseNotes.mjs + CLI tools/release-notes.mjs, npm run release:notes) for the first MVP proof-of-concept candidate. It folds the curated "what\'s built" narrative (shooter proof loop, Nostr read/profile/leaderboard surfaces, gateway travel shell, update/VPS readiness, Continuum dashboard, SDK/debug handoff surfaces, tests/guardrails, known non-blocking advisories) with the existing local verdicts — the MVP readiness rollup + the release-candidate gate + the handoff brief — and emits a DRAFT (text/--json/--markdown, opt-in bounded in-repo --write → RELEASE_NOTES_DRAFT.md). DRAFT ONLY: no GitHub release, no git tag, no public announcement, no network. +13 unit tests. NON-GOALS held: read-only; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
  ...
  // Now / Next / Later.
  activeNow: [
    'v0.2.202 — MVP PROOF RELEASE NOTES DRAFT (docs/tooling only, no runtime change): a pure/local release-notes draft (tools/releaseNotes.mjs + CLI tools/release-notes.mjs, npm run release:notes) for the first MVP proof-of-concept candidate. It folds the curated "what\'s built" narrative — shooter proof loop, Nostr read/profile/leaderboard proof surfaces, gateway travel shell, update/VPS readiness, Continuum dashboard, SDK/debug handoff surfaces, tests/guardrails, known non-blocking advisories — with the already-computed local verdicts (the MVP readiness rollup + the release-candidate gate + the handoff brief) and emits a DRAFT in text / --json / --markdown, plus an opt-in bounded in-repo --write (default RELEASE_NOTES_DRAFT.md, confined via the shared resolveHandoffWritePath boundary). DRAFT ONLY: it creates no GitHub release, no git tag, no public announcement, and reaches no network/server. +13 unit tests. NON-GOALS held: read-only; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.',
    'v0.2.201 — MVP RELEASE-CANDIDATE GATE (infra/tooling only, no runtime change): a pure/local gate (tools/mvpRcGate.mjs + CLI tools/mvp-rc-gate.mjs, npm run rc:gate) composing the MVP readiness rollup + the release-readiness summary into ONE READY / NEAR / BLOCKED verdict + a blended percentage + the blocking reasons + the next one or two safe tasks. Read-only oversight that PREVIEWS npm run test:release; creates no release/tag/deploy/publish/upload. +14 unit tests.',
