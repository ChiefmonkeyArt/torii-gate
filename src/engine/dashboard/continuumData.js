...
export const CONTINUUM_VERSION = 'v0.2.200-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.200-alpha (build truth; live trails — manual deploy)' },
    ...
    { label: 'Active slice', value: 'v0.2.200 DASHBOARD METRIC FRESHNESS CLEANUP (infra/dashboard data-quality only, no runtime change) — resolves the recurring stale LAST-KNOWN test-count flagged in security reviews (HEALTH_LASTKNOWN.totalTests was a second hand-maintained copy stuck at "1180 passing" while the suite had moved on). A single curated source of truth (CURRENT_TEST_STATUS) + a testCountLabel() helper now feed BOTH displayed surfaces — the "at a glance" Tests metric AND the engineering-health Total tests metric — so they can no longer drift apart. New unit tests assert the curated FILE count equals the real on-disk test-file count (forcing a bump when files change), that both labels derive from the one source, and that the dashboard capture agrees with the MVP-rollup capture (mvpReadiness.DEFAULT_TEST_STATUS). NON-GOALS held: read-only data cleanup; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
