...
export const CONTINUUM_VERSION = 'v0.2.197-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.197-alpha (build truth; live trails — manual deploy)' },
    { label: 'Tests', value: '1214 passing / 76 files (profiles: test:fast ~5, test:foundation ~25)' },
    ...
    { label: 'Active slice', value: 'v0.2.197 HOST ROUTE SMOKE (infra/testing/tooling only, no runtime change) — a pure, node-safe read-only smoke harness (engine/host/hostRouteSmoke.js) that folds the shipped static-host readiness contracts into ONE fail-fast report so future torii.quest / VPS static-host work can be regression-checked locally without a server, shell, DNS, SSH, or network: root index.html (the SPA entry / fallback document) present; DIST_SPEC artifacts (index.html + assets/) present; the continuum dashboard page + data JSON present; release-metadata.json present AND manual-only/non-actionable; the release REQUIRED_FILES floor intact; the /zone/* SPA fallback documented in VPS_INSTALL.md/HANDOFF.md; no built file shadows the /zone/* fallback; an unknown /zone/<slug> is NOT a built file so the host serves index.html while the app parser keeps the slug safe; the route parser classes a good slug ZONE and every hostile path (traversal/scheme/protocol-relative/uppercase) INVALID; and every exercised report pins served/deployed/navigated/performed/external/network/wrote/fetched=false. It inspects deterministic LOCAL fixtures (a built-path list + doc text) only — it touches NO server/DNS/SSH/network and renders/deploys NOTHING. Surfaced via the SDK (hostRouteSmoke, experimental), the debug shell (ToriiDebug.shells.hostRouteSmoke), and folded into buildShellReport. +17 unit tests. NON-GOALS held: NOT a VPS deployment — touches no real server; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
