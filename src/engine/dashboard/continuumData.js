...
export const CONTINUUM_VERSION = 'v0.2.192-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.192-alpha (build truth; live trails — manual deploy)' },
    { label: 'Tests', value: '1100 passing / 71 files (profiles: test:fast ~5, test:foundation ~25)' },
    ...
    { label: 'Active slice', value: 'v0.2.192 GITHUB RELEASE METADATA (tooling/docs only, no runtime change) — prepares the static metadata a FUTURE torii.quest / VPS update-checker reads to show an inert "update available" notice, without any live update or runtime network. Pure buildReleaseMeta() shapes {kind, schemaVersion, channel (derived from the version tag), version, commit, source GitHub URLs (doc-only), dist artifact expectations, requiredFiles/Checks, and manual/no-auto-update consent wording}; validateReleaseMeta() enforces the safety floor — autoUpdate/actionable MUST be false (ERROR if not). Thin CLI npm run release:meta (text default; --json; --write the deterministic public/release-metadata.json; --stamp bakes live commit/time). Local/read-only/no-network; ALWAYS exits 0. +23 unit tests. NON-GOALS held: no live update execution, no gameplay/portal/physics/shooting/controls or live Nostr write change.' },
  ],
  ...
  leanRoute: [
    ...
    { id: 'LEAN-5', state: 'in-progress', progress: 55, slice: 'torii.quest GitHub update-check', status: 'helper + view-model + release source/status + static release-metadata template/spec (v0.2.192, npm run release:meta); needs read-only releases fetch + prompt mesh' },
  ],

  // Now / Next / Later.
  activeNow: [
    'v0.2.192 — GITHUB RELEASE METADATA (tooling/docs only, no runtime change): prepares the static metadata a FUTURE torii.quest / VPS update-checker reads to surface an inert "update available" notice, with NO live update and NO runtime network. Pure buildReleaseMeta() (tools/releaseMeta.mjs) shapes {kind, schemaVersion, channel derived from the version tag, version, commit, documentation-only GitHub source URLs, dist artifact expectations, requiredFiles/requiredChecks, and manual/no-auto-update consent + notice wording}; validateReleaseMeta() is the safety floor — update.autoUpdate and update.actionable MUST be false or it ERRORs. Thin CLI npm run release:meta (text default; --json; --write emits the DETERMINISTIC public/release-metadata.json so re-runs never churn the tree; --stamp bakes live commit/time for a deploy step). Local/read-only/network-free, ALWAYS exits 0. +23 unit tests. NON-GOALS held: no live update execution, no gameplay/portal/physics/shooting/controls or live Nostr write change.',
