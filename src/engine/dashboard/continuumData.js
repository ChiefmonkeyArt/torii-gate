...
export const CONTINUUM_VERSION = 'v0.2.242-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.242-alpha (build truth; live trails — manual deploy)' },
    ...
    { label: 'Active slice', value: 'v0.2.242 ZONE EXACT-PATH DEEP-LINK REPAIR (game slice) — fixes the v0.2.241 follow-up: the live smoke STILL 404d a hard-refresh / deep-link of /zone/plebeian-market-bazaar. ROOT CAUSE: the static host (torii-quest.pplx.app) matches files by EXACT path with NO SPA rewrite AND no directory-index resolution, so it never maps the extensionless /zone/<slug> URL onto the v0.2.241 nested dist/zone/<slug>/index.html shell — the cold hit returned the host JSON 404 ("No static asset at /zone/...") before the v0.2.182 client parser could run. FIX (no backend): the build now writes the byte-identical shell to the EXACT-PATH extensionless file dist/zone/<slug> (the precise path the host resolves for the no-trailing-slash URL) for each DEPLOYABLE_ZONE_SLUGS entry (single source of truth in zoneRoute.js), via the pure planner (tools/zoneShells.mjs) + fs generator (tools/generate-zone-shells.mjs) wired into npm build. A file dist/zone/<slug> and a directory dist/zone/<slug>/ cannot coexist, so this REPLACES the v0.2.241 directory-index form. index.html uses root-absolute /assets/* URLs, so the shell loads the same bundle and the parser resolves the slug exactly as for an in-world portal hop. The zone-fallback readiness guard (zoneFallbackReadiness checkDistRoutes + zones:check + regression [15] + build-continuum) was reconciled to ALLOW the verified exact-path shell (/zone/<slug> byte-identical to index.html) while still failing on any other /zone/* file that would shadow the fallback. RESIDUAL RISK (needs live re-smoke by the parent): an extensionless file may be served with a non-HTML Content-Type by the host — unverifiable locally. Preserves the v0.2.240 service-worker fail-soft precache (zone shells are HTML → network-first), the v0.2.238 fail-closed loop, and the v0.2.236 NIP-07 login decoupling; root entry flow + ESC pause unchanged. Prior — v0.2.241 zone hard-refresh shell (directory-index, superseded); v0.2.240 travel-gateway entry repair; v0.2.239 travel-gateway placement; v0.2.238 ENTER-ARENA reset-crash fix; v0.2.237 workflow-invariant slice. HARD CONSTRAINTS held: godMode false; no new timers (loop uses rAF only); no new hot-path Vector3/Matrix4; nostrich comments; Chiefmonkey exact; debug tools ship unconditionally; non-religious ethics guard + useful-job invariant intact; no Nostr writes/signing beyond the existing login/read; no deploy/publish/push (parent handles those).' },
  ],
