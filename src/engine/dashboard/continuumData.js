...
export const CONTINUUM_VERSION = 'v0.2.205-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.205-alpha (build truth; live trails — manual deploy)' },
    ...
    { label: 'Active slice', value: 'v0.2.205 PLAYTEST CHECKLIST DOC WARNING CLEANUP (docs-only, no runtime change) — cleared the non-blocking WARN from the v0.2.204 security review: two illustrative v0.2.203-alpha example strings (plus a historical authorship stamp) lingered in tools/playtestChecklist.mjs comments/examples. The version-label example and the buildPlaytestChecklistModel version-input comment are now version-neutral (a vX.Y.Z-alpha marker) so future security reviews no longer re-flag stale examples. No behavior change; no test count change (no new test file). NON-GOALS held: docs/comment cleanup only; no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
