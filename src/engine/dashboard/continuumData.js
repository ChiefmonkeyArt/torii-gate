...
export const CONTINUUM_VERSION = 'v0.2.195-alpha';
...
export const CONTINUUM = Object.freeze({
  ...
  // "At a glance" metrics.
  metrics: [
    { label: 'Source version', value: 'v0.2.195-alpha (build truth; live trails — manual deploy)' },
    { label: 'Tests', value: '1180 passing / 74 files (profiles: test:fast ~5, test:foundation ~25)' },
    ...
    { label: 'Active slice', value: 'v0.2.195 GATEWAY TRAVEL SMOKE (infra/testing/tooling only, no runtime change) — a pure, node-safe read-only smoke harness (engine/gateway/travelSmoke.js) that folds the shipped travel-flow contracts into ONE fail-fast report so future portal/travel work can be regression-checked locally without a browser: trigger arms on proximity but proximity ALONE never navigates; an explicit confirm is required; the hop targets a same-origin /zone/<slug> route only; the allowlist is scoped (never "/"); a valid /zone/<slug> resolves while traversal/scheme/protocol-relative routes are rejected; no external gateway website is carried; consent gates travel; and every exercised report pins navigated/performed/external/signed/published/network=false. It drives the boundary with dryRun:true and NO injected transport, so a fully-confirmed confirm() is a dry-run no-op that navigates NOTHING. Surfaced via the SDK (travelSmoke, experimental), the debug shell (ToriiDebug.shells.travelSmoke), and folded into buildShellReport. +12 unit tests. NON-GOALS held: no gameplay/physics/shooter/Rapier change; no Nostr signing/publishing/live network write; godMode stays false.' },
  ],
