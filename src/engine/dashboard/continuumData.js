...
export const CONTINUUM = Object.freeze({
  ...
  completed24h: [
    'v0.2.189 — release-readiness JSON EXPORT (tooling/docs only, no runtime change): the local ship verdict is now machine-readable. A pure buildReleaseStatusJson() wraps the existing releaseReadiness summary in a stable schema envelope (schema/schemaVersion + status, ready, blockers, unknowns, per-signal map, version/packageVersion/gitCommit, latestReports) so the dashboard/handoff/updater/agents can consume readiness without parsing human terminal text. Deterministic by design: generatedAt is the ONLY non-deterministic field and is optional + isolated (omit → null for reproducible tests; the CLI stamps a real ISO time). Garbled/non-object input → honest {status:unknown, error:no-summary}; never throws, never mutates source. New npm run release:status:json + node tools/release-readiness.mjs --json (raw stdout is pure JSON; npm run banner to stderr). +7 unit tests. NON-GOALS: no fs writes from the game, no network, no deploy/publish, no gameplay/portal/physics/controls/Nostr change.',
