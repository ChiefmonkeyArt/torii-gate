...
Current version: `v0.2.124-alpha`  
...
| Shots "on target" miss, especially at distance | `ToriiDebug.combat.lastShot`/`lastMiss` — `reason: 'moved-or-offset'` ⇒ projectile travel time / barrel-convergence offset (reticle is camera-hitscan, bullet is barrel-fired); `'blocked'` ⇒ geometry in the way; `'aim-off'` ⇒ crosshair wasn't on a bot; `'head-to-body'` ⇒ headshot dropped to torso |
