...
Current version: `v0.2.116-alpha`  
...
| Event bus | `src/events.js` | Decoupling seam — modules publish/subscribe by name instead of importing each other. `EV` is the canonical registry; `on/off/emit`. Imports nothing (no cycles). v0.2.116 wired `EV.PHASE_CHANGE` (payload `{from,to,event}`) from `state.transition()` and added check 8 (every `EV.<NAME>` reference must be defined here). Dormant seams: `PHASE_CHANGE` (no subscriber yet), `WS_PLAYER_HIT`, `WS_CHAT`. |
...
- **Event bus (continue)**: first slice formalised v0.2.116 (see Stable/started). Remaining: migrate the remaining direct cross-module couplings onto the bus — the `window._onBotHit` global bridge (weapons.js → main.js) and the `window._grassMat`/`window._flowerMat` shader refs — add subscribers for `PHASE_CHANGE` (HUD/audio/presence) and emit `WS_*` from the netcode skeleton when it lands.
