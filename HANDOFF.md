...
- **Current version:** v0.2.220-alpha (see §3 for every place the version string lives)
...
| `MVP_APPROVAL_STATE.json` | `version` field — generated by `npm run approval:state -- --write`; `mvp-approval-state.test.js` asserts the committed (pending) record tracks `config.js` `VERSION` (v0.2.220) |
