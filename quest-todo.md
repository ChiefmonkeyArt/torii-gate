...
Remaining items from the v0.2.259 code audit (`torii-quest-audit-v0.2.259.md`). Already landed in v0.2.260: E1 (vitest pool=threads, 28.7s→2.7s), GLB1+GLB2 (−66% assets, DRACOLoader), R3 (SW precache trim), S5 (relay fan-out), S2 (port hardening), repo hygiene. Landed in v0.2.261: **R1** (dashboard SDK split — app chunk −19.4% raw / −19.7% gzip). Landed in v0.2.262: **S1** (BIP-340 schnorr verification across SEC-1 + SEC-2 — crypto-verified trust tier + relay-spoof / forged-sig / tampered-event tests).

Each item is sized as a single thin slice. Keep them in priority order; pick one per session.

- **S1 — schnorr verify in handoffVerify — DONE v0.2.262 (`s1-schnorr-verification` branch, pending user merge).** Built `src/engine/crypto/nostrSig.js` (pure `computeEventId` + `verifyEventSignature` over `@noble/curves/secp256k1` + `@noble/hashes/sha2`). Wired opt-in `cryptoVerify` into `handoffVerify.js` (new `'crypto-verified'` tier) + `publishGate.js` (tightened sig from hex64 to hex128; new `'crypto-verified'` tier; structural floor protects against malformed inputs first). `handshakeController.tick` now passes `rawEvent` + `cryptoVerify:true` so the live n2n hop arms only on a real BIP-340-verified accept. `main.js _executeJump` re-runs the gate one more time right before `window.location.href` — the single navigation site stays the single crypto bottleneck. 1856 tests (108 files) green; app bundle 245.2 KB → 277.0 KB raw / +12.5 KB gzip for full schnorr crypto.
