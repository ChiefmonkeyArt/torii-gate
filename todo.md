...
- **The active MVP is the freedom-tech loop**, four demonstrable slices:
  1. **Gateway / NAP-to-NAP preview** — the Torii Gateway portal view + travel-intent
     ...
     pinned, and it exposes NO bare browser-navigation method). Real relay read transport, the
     in-world gateway/destination MESH, the clickable confirm dialog that MINTS a grant, and the
     REAL host transport wiring (router/history) that this executor will drive remain the
