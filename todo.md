...
> Live site: [torii-quest.pplx.app](https://torii-quest.pplx.app) | Current version: **v0.2.124-alpha**
...
| COMBAT-HITREG | TQ | COMBAT | **Distance hit-registration fix — DIAGNOSTICS LANDED (v0.2.124), FIX PENDING manual data.** User feedback: head/body shots "appear on target" but miss, especially at distance. v0.2.124 added per-shot diagnostics (`ToriiDebug.combat.lastShot`/`lastMiss`, pure `engine/combat/shotDiagnostics.js` miss-reason classifier) WITHOUT changing gameplay. Root cause documented: the reticle is an instantaneous CAMERA hitscan ray while the bullet is a BARREL-fired projectile (lateral/vertical barrel offset that only zeroes at the 80 m convergence point, PLUS finite travel time so moving bots leave the path at range). **Next (needs manual `lastShot` data to pick the fix):** options are (a) tighten/zero the camera↔barrel parallax by firing the bullet line straight down the camera ray (origin still camera-side per the camera-bullet rule), (b) increase `BULLET_SPEED` to shrink lead error, (c) add target-lead or a thin hit-assist radius for bots at range, or (d) make the reticle predict the projectile (not hitscan) so preview matches outcome. Do NOT blindly loosen head/body thresholds. |
